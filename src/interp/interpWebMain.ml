                   ...
                   let ppf = Format.formatter_of_buffer myBuf in
                     ...
                     match msg with
                       ...
                       begin
                         ...
                         let string_of_value : type a. a itag * a -> string = function
                           | Number Real, v -> string_of_float v
                           | Int8, v -> string_of_int v
                           | Int16, v -> string_of_int v
                           | Int32, v -> string_of_int v
                           | Uint8, v -> string_of_int v
                           | Uint16, v -> string_of_int v
                           | Uint32, v -> string_of_int v
                           | Bool, v ->  string_of_bool v
                           | String, v -> Printf.sprintf "%S" v
                           | ty, v ->
                             Buffer.clear myBuf;
                             InterpMessages.print_value ppf (inject (Single ty) v);
                             Format.fprintf ppf "%!" ;
                             Buffer.contents myBuf
                           | _ -> "(string_of_value not implemented)"
                         in
                         ...
                         let matrix_to_table m ty =
                           ...
                           for i=1 to h do
                             for j=1 to w do
                               td_list := !td_list @ [(D.td [ D.pcdata (string_of_value (ty, matrix_get m j i)) ])]
                             done;
                             tr_list := !tr_list @ [D.(tr !td_list)] ;
                             td_list := [];
                           done;
                           D.([table ~a:[a_class ["scilab-matrix"] ] !tr_list])
                         in
                         ...
                         let format_vlist l =
                           ...
                           for i=1 to vlist_length l do
                             (match view (vlist_get l i) with
                              | V (Matrix ty, m) -> res := !res @ (matrix_to_table m ty)
                              | _ ->
                                Buffer.clear myBuf ;
                         ...
                         let formatted_result = match view v with
                           ...
                           | _ ->  Buffer.clear myBuf;
                                 InterpMessages.print_value ppf v;
                                 Format.fprintf ppf "%!" ;
                                 D.([ pcdata (Buffer.contents myBuf) ])
                         in
